    ...
    <div className="bg-background text-foreground">
      ...
      <Schema
        schema={buildWebPageSchema(
          "SABACHO Marani — Georgian Wine Experience in Kakheti",
          "Private Georgian winery in Kakheti. Wine tasting, Supra feasts, Chacha and aged Cognac. Reserve your Sabacho experience.",
          "/",
        )}
      />
  ...
  return (
    ...
    >
      <div className="absolute inset-0">
        {/* Static poster image loads instantly as LCP element */}
        <img
          src={HERO_FALLBACK}
          ...
          fetchPriority="high"
          decoding="async"
          className={`w-full h-full object-cover animate-kenburns transition-opacity duration-700 ${videoReady ? "opacity-0" : "opacity-100"}`}
...
function Badge({ children }: { children: React.ReactNode }) {
  return (
    <span className="border border-gold/40 text-gold px-3 py-1.5 rounded-full text-[11px] uppercase tracking-[0.2em]">
      {children}
    </span>
  );
}
        ...
        <div className="mt-8 relative overflow-hidden rounded-2xl border border-gold/40 bg-gradient-to-br from-gold/10 via-background to-background p-8 md:p-12">
          ...
          <div className="relative flex flex-col md:flex-row md:items-center gap-6">
            ...
            <div className="flex-1">
              <h3 className="font-serif text-3xl md:text-4xl text-gold">
                {t("exp.private.title")}
              </h3>
      ...
      <DetailOverlay
        ...
        sections={[
          ...
          },
          {
            label: "Atmosphere",
            value:
              "Candlelit stone cellar, hand-carved wooden table, family host guiding every pour.",
          },
          {
            label: "Private option",
            value:
              "Reserve in advance to make the evening yours alone — with wines chosen specially for your group.",
          },
        ]}
        onBook={scrollToBook}
      ...
      <DetailOverlay
        ...
        sections={[
          ...
          },
          {
            ...
            value:
              "Toasts to peace, to family, to guests — a tradition that turns strangers into friends by the end of the evening.",
          },
          {
            label: "The table",
            value:
              "Home-cooked Kakhetian dishes made by the family, served under the vine-covered gazebo or in the marani.",
          },
        ]}
        onBook={scrollToBook}
  ...
  return (
    ...
    >
      <div className="aspect-[16/10] overflow-hidden">
        ...
        <span className="absolute bottom-4 left-6 text-[10px] uppercase tracking-[0.3em] text-white/80 opacity-0 group-hover:opacity-100 transition-opacity">
          Discover →
        </span>
      </div>
      <div className="p-6 md:p-8">
        <div className="flex items-center gap-2 text-gold">
          ...
          {price && (
            <span className="ml-auto text-sm font-medium text-gold whitespace-nowrap">{price}</span>
          )}
        </div>
        ...
        <div className="relative p-8 md:p-16 grid md:grid-cols-2 gap-10 items-center">
          <div>
            ...
            <h2 className="font-serif text-5xl md:text-6xl text-foreground">
              {t("tasting.teaser.title")}
            </h2>
            <p className="mt-4 text-muted-foreground max-w-md leading-relaxed">
              {t("tasting.teaser.subtitle")}
            </p>
            <a
              href="/tasting"
  ...
  const items = [
    {
      ...
      rating: 5,
      review:
        "The best evening of our Georgia trip. The family hosted us like their own — cellar tour, unforgettable wine, a Supra that lasted hours.",
    },
    {
      name: "Daniel Peters",
      country: "Germany",
      rating: 5,
      review:
        "Authentic in a way you can't fake. The qvevri wine and 10-year cognac were extraordinary. Highly recommend booking the private evening.",
    },
    {
      ...
      rating: 5,
      review:
        "Дуже теплий прийом, справжня грузинська гостинність. Вина неймовірні, чача — окрема пісня. Обов'язково повернемось.",
    },
    {
      name: "Emma R.",
      ...
      rating: 5,
      review:
        "Beautiful garden, candlelit stone cellar, wines with real character. This is what people mean when they say 'Georgian hospitality'.",
    },
  ];
  return (
    <section
          ...
          {items.map((it, i) => (
            <div key={i} className="border border-border/60 rounded-lg overflow-hidden">
              ...
              >
                ...
                <ChevronRight
                  className={`w-4 h-4 text-gold transition-transform ${open === i ? "rotate-90" : ""}`}
                  aria-hidden="true"
                />
              </button>
              {open === i && (
    ...
    >
      <div className="max-w-5xl mx-auto text-center">
        ...
        <p className="text-sm text-muted-foreground max-w-xl mx-auto">
          <a href={`tel:${PHONE}`} className="hover:text-gold" aria-label={`Call us at ${PHONE}`}>
            +995 {PHONE}
          </a>{" "}
          ·{" "}
          <a
