      ...
      <div className="w-full max-w-md">
        ...
        <div className="glass rounded-2xl p-8">
          ...
          <form onSubmit={submit} className="space-y-4">
            <input type="email" required value={email} onChange={e => setEmail(e.target.value)} placeholder={t("admin.email")} className="w-full bg-background/40 border border-border focus:border-gold outline-none rounded px-4 py-3 text-sm" />
            <input type="password" required minLength={8} value={password} onChange={e => setPassword(e.target.value)} placeholder={t("admin.password")} className="w-full bg-background/40 border border-border focus:border-gold outline-none rounded px-4 py-3 text-sm" />
            <button disabled={loading} className="w-full bg-carrot hover:bg-carrot-hover text-white py-3 rounded text-xs uppercase tracking-[0.25em] disabled:opacity-60">
              {loading ? "…" : t("admin.signin")}
            </button>
          </form>
          <button onClick={() => setMode(m => m === "signin" ? "signup" : "signin")} className="mt-4 text-xs text-muted-foreground hover:text-gold w-full text-center">
