        ...
        <div className="max-w-5xl mx-auto">
              ...
              <div className="relative flex flex-col md:flex-row gap-5 md:gap-8">
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-6 flex flex-wrap gap-3">
                    <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="bg-[#25D366] text-white px-6 py-3 rounded-md text-xs uppercase tracking-[0.25em] hover:brightness-110 transition">
                      {t("restoration.whatsapp")}
                    </a>
                    <a href={`tel:${PHONE_INTL}`} className="border border-gold text-gold hover:bg-gold hover:text-black px-6 py-3 rounded-md text-xs uppercase tracking-[0.25em] transition">
                      {t("restoration.call")}
                    </a>
                  </div>
                </div>
          ...
          <div className="grid md:grid-cols-2 gap-10 items-start">
            <div className="space-y-6">
              <ContactRow icon={<MapPin className="w-5 h-5" />} label={t("contact.location_label")} value={t("contact.address")} href={MAPS_URL} external />
              <ContactRow icon={<Phone className="w-5 h-5" />} label={t("contact.phone_label")} value={`+995 ${PHONE}`} href={`tel:${PHONE_INTL}`} />
              <ContactRow icon={<Mail className="w-5 h-5" />} label={t("contact.email_label")} value={EMAIL} href={`mailto:${EMAIL}`} />
              <div className="border-t border-border/60 pt-6">
                ...
                <div className="flex flex-wrap gap-3 text-xs uppercase tracking-[0.2em]">
                  <a href={SOCIALS.instagram} target="_blank" rel="noopener noreferrer" className="border border-border/60 hover:border-gold hover:text-gold px-4 py-2 rounded-full transition">Instagram</a>
                  <a href={SOCIALS.facebook} target="_blank" rel="noopener noreferrer" className="border border-border/60 hover:border-gold hover:text-gold px-4 py-2 rounded-full transition">Facebook</a>
                  <a href={SOCIALS.tiktok} target="_blank" rel="noopener noreferrer" className="border border-border/60 hover:border-gold hover:text-gold px-4 py-2 rounded-full transition">TikTok</a>
                  <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer" className="border border-border/60 hover:border-gold hover:text-gold px-4 py-2 rounded-full transition">WhatsApp</a>
                </div>
              </div>
              <div className="pt-4">
...
function ContactRow({ icon, label, value, href, external }: { icon: React.ReactNode; label: string; value: string; href: string; external?: boolean }) {
  return (
    <a href={href} target={external ? "_blank" : undefined} rel={external ? "noopener noreferrer" : undefined} className="flex items-start gap-4 group">
      <span className="w-10 h-10 rounded-full bg-gold/10 text-gold flex items-center justify-center shrink-0 border border-gold/30 group-hover:bg-gold group-hover:text-black transition">{icon}</span>
