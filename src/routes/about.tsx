  ...
  head: () => ({
    meta: [
      ...
      },
      {
        name: "keywords",
        content:
          "Sabacho story, Georgian winery family, Kakheti marani, Georgian wine tradition, Sabacho Marani history, Georgian wine family",
      },
...
type AboutContent = {
  ...
  hero_image?: string;
  owner_name?: string;
  owner_name_ka?: string;
  owner_name_ru?: string;
  owner_name_uk?: string;
  owner_role?: string;
  owner_role_ka?: string;
  owner_role_ru?: string;
  owner_role_uk?: string;
  owner_bio?: string;
  owner_bio_ka?: string;
  owner_bio_ru?: string;
  owner_bio_uk?: string;
  ...
  marani_title?: string;
  marani_title_ka?: string;
  marani_title_ru?: string;
  marani_title_uk?: string;
  marani_body?: string;
  marani_body_ka?: string;
  marani_body_ru?: string;
  marani_body_uk?: string;
  marani_image?: string;
  village_title?: string;
  village_title_ka?: string;
  village_title_ru?: string;
  village_title_uk?: string;
  village_body?: string;
  village_body_ka?: string;
  village_body_ru?: string;
  village_body_uk?: string;
    ...
    <div className="bg-background text-foreground">
      ...
      <main>
          ...
          <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-12 md:gap-16 items-center">
            ...
            <div>
              <div className="text-[10px] uppercase tracking-[0.4em] text-gold mb-3">
                {t("about.marani_eyebrow")}
              </div>
              <h2 className="font-serif text-4xl md:text-5xl lg:text-6xl font-light leading-tight">
                {maraniTitle}
              </h2>
              <p className="mt-6 text-muted-foreground leading-relaxed md:text-lg whitespace-pre-line">
                {maraniBody}
              </p>
            </div>
          </div>
        ...
        <section
          ...
          aria-label={t("about.people_title")}
        >
        ...
        >
          <div className="max-w-3xl mx-auto text-center">
            <h2 className="font-serif text-4xl md:text-6xl font-light leading-tight text-gold">
              {ctaTitle}
            </h2>
      ...
      <BreadcrumbSchema
        items={[
          ...
          { name: "Our Story", item: "/about", position: 2 },
        ]}
      />
      <Schema
        schema={buildWebPageSchema(
          "Our Story | SABACHO Marani",
          "The story of Sabacho Marani — the family, the marani, the village, and the Georgian hospitality behind every glass.",
          "/about",
        )}
      />
...
function ProfileCard({
  ...
  return (
    <div className="text-center">
      ...
      {bio && (
        <p className="mt-3 text-sm text-muted-foreground leading-relaxed max-w-xs mx-auto">{bio}</p>
      )}
    </div>
  );
}
