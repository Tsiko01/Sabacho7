...
type Faq = {
  id: string;
  question: string; question_ka: string | null; question_ru: string | null; question_uk: string | null;
  answer: string; answer_ka: string | null; answer_ru: string | null; answer_uk: string | null;
...
function FaqsAdmin() {
  ...
  const { data } = useQuery({
    queryKey: ["admin-faqs"],
    queryFn: async () => (await supabase.from("faqs").select("*").order("sort_order")).data as unknown as Faq[] ?? [],
  });
  const items = data ?? [];
  ...
  const add = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-faqs"] }); qc.invalidateQueries({ queryKey: ["faqs"] }); toast.success("Added"); },
  ...
  return (
    <div className="max-w-5xl">
      ...
      <div className="space-y-4">
        ...
        {items.length === 0 && <div className="text-sm text-muted-foreground border border-dashed border-border/60 rounded-lg p-8 text-center">No FAQs yet. Add your first one.</div>}
      </div>
    </div>
  );
}
...
function FaqEditor({ row }: { row: Faq }) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("faqs").update({
        question: form.question, question_ka: form.question_ka, question_ru: form.question_ru, question_uk: form.question_uk,
        answer: form.answer, answer_ka: form.answer_ka, answer_ru: form.answer_ru, answer_uk: form.answer_uk,
        sort_order: form.sort_order, active: form.active,
      }).eq("id", row.id);
      if (error) throw error;
    },
    onSuccess: () => { toast.success("Saved"); qc.invalidateQueries({ queryKey: ["admin-faqs"] }); qc.invalidateQueries({ queryKey: ["faqs"] }); },
    onError: (e) => toast.error((e as Error).message),
  });
  ...
  const del = useMutation({
    ...
    onSuccess: () => { toast.success("Deleted"); qc.invalidateQueries({ queryKey: ["admin-faqs"] }); qc.invalidateQueries({ queryKey: ["faqs"] }); setAskDelete(false); },
    ...
    <div className="border border-border/60 rounded-xl bg-card">
      ...
      {open && (
        <div id={`faq-editor-${row.id}`} className="p-5 border-t border-border/60 grid gap-3 md:grid-cols-2">
          ...
          <input className={inp} placeholder="Question (RU / Русский)" value={form.question_ru ?? ""} onChange={e => setForm(f => ({ ...f, question_ru: e.target.value }))} />
          <input className={inp} placeholder="Question (UK / Українська)" value={form.question_uk ?? ""} onChange={e => setForm(f => ({ ...f, question_uk: e.target.value }))} />

          <div className="md:col-span-2 mt-2 text-[10px] uppercase tracking-[0.3em] text-gold">Answer</div>
          <textarea className={`${inp} md:col-span-2 min-h-24`} placeholder="Answer (EN)" value={form.answer} onChange={e => setForm(f => ({ ...f, answer: e.target.value }))} />
          <textarea className={`${inp} min-h-24`} placeholder="Answer (KA / ქართული)" value={form.answer_ka ?? ""} onChange={e => setForm(f => ({ ...f, answer_ka: e.target.value }))} />
          <textarea className={`${inp} min-h-24`} placeholder="Answer (RU / Русский)" value={form.answer_ru ?? ""} onChange={e => setForm(f => ({ ...f, answer_ru: e.target.value }))} />
          <textarea className={`${inp} min-h-24 md:col-span-2`} placeholder="Answer (UK / Українська)" value={form.answer_uk ?? ""} onChange={e => setForm(f => ({ ...f, answer_uk: e.target.value }))} />

          <input className={inp} type="number" placeholder="Sort order" value={form.sort_order} onChange={e => setForm(f => ({ ...f, sort_order: Number(e.target.value) }))} />
