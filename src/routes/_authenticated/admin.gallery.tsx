...
type Row = { id: string; image_url: string; alt: string | null; category: string | null; sort_order: number; active: boolean };
...
function GalleryAdmin() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => (await supabase.from("gallery_images").select("*").order("sort_order")).data as unknown as Row[] ?? [],
  });
  const rows = data ?? [];

  const addFromUrl = useMutation({
    mutationFn: async ({ url, category }: { url: string; category: string }) => {
      const { error } = await supabase.from("gallery_images").insert({ image_url: url, category, sort_order: (rows.at(-1)?.sort_order ?? 0) + 1 });
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-gallery"] }); qc.invalidateQueries({ queryKey: ["gallery"] }); toast.success("Added"); setUrlInput(""); },
  ...
  async function uploadFiles(files: FileList | null) {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        const { error: insErr } = await supabase.from("gallery_images").insert({ image_url: url, alt: file.name, category: urlCat, sort_order: (rows.at(-1)?.sort_order ?? 0) + 1 });
  ...
  const del = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-gallery"] }); qc.invalidateQueries({ queryKey: ["gallery"] }); toast.success("Deleted"); setPendingDelete(null); },
    onError: (e) => { toast.error((e as Error).message); setPendingDelete(null); },
  });
    ...
    <div className="max-w-6xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Add, remove and order photos shown on the public Gallery page.</p>
        </div>
      </div>
      ...
      {rows.length === 0 ? (
        <div className="border border-dashed border-border/60 rounded-2xl p-14 text-center">
          ...
          <p className="text-sm text-muted-foreground mt-2">Upload or add photos to build the public gallery.</p>
        </div>
      ) : (
...
function GalleryTile({ row, onDelete }: { row: Row; onDelete: () => void }) {
  ...
  const save = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("gallery_images").update({ alt, active, sort_order: order, category }).eq("id", row.id);
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["admin-gallery"] }); qc.invalidateQueries({ queryKey: ["gallery"] }); toast.success("Saved"); },
    onError: (e) => toast.error((e as Error).message),
  });
