...
function AboutAdmin() {
  ...
  useEffect(() => { if (aboutData) setAbout(aboutData); }, [aboutData]);
  useEffect(() => { if (restData) setRest(restData); }, [restData]);

  const saveAbout = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("site_content").upsert({ key: "about_page", value: about }, { onConflict: "key" });
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["site_content", "about_page"] }); toast.success("Saved"); },
    onError: (e) => toast.error((e as Error).message),
  });
  const saveRest = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("site_content").upsert({ key: "restoration", value: rest }, { onConflict: "key" });
      if (error) throw error;
    },
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["site_content", "restoration"] }); toast.success("Saved"); },
    onError: (e) => toast.error((e as Error).message),
  });

  const inp = "w-full bg-background/40 border border-border/60 focus:border-gold outline-none rounded px-3 py-2 text-sm";
  const set = <K extends keyof AboutContent>(k: K, v: AboutContent[K]) => setAbout(a => ({ ...a, [k]: v }));
  ...
  return (
    <div className="max-w-4xl space-y-12">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Edit the story, people and images on the public About page.</p>
      </div>
      ...
      <Section title="Hero">
        <input className={inp} placeholder="Hero title" value={about.hero_title ?? ""} onChange={e => set("hero_title", e.target.value)} />
        <textarea className={`${inp} min-h-20`} placeholder="Hero subtitle" value={about.hero_subtitle ?? ""} onChange={e => set("hero_subtitle", e.target.value)} />
        <input className={inp} placeholder="Hero image URL (optional)" value={about.hero_image ?? ""} onChange={e => set("hero_image", e.target.value)} />
      </Section>

      <Section title="Owner">
        <div className="grid gap-3 md:grid-cols-2">
          <input className={inp} placeholder="Owner name" value={about.owner_name ?? ""} onChange={e => set("owner_name", e.target.value)} />
          <input className={inp} placeholder="Owner role" value={about.owner_role ?? ""} onChange={e => set("owner_role", e.target.value)} />
        </div>
        <textarea className={`${inp} min-h-24`} placeholder="Owner bio" value={about.owner_bio ?? ""} onChange={e => set("owner_bio", e.target.value)} />
        ...
        {hosts.map((h, i) => (
          <div key={i} className="border border-border/60 rounded-lg p-4 space-y-2">
            ...
            <div className="grid gap-2 md:grid-cols-2">
              ...
              <input className={inp} placeholder="Role" value={h.role} onChange={e => setAbout(a => ({ ...a, hosts: (a.hosts ?? []).map((x, j) => j === i ? { ...x, role: e.target.value } : x) }))} />
            </div>
            <textarea className={`${inp} min-h-20`} placeholder="Bio" value={h.bio} onChange={e => setAbout(a => ({ ...a, hosts: (a.hosts ?? []).map((x, j) => j === i ? { ...x, bio: e.target.value } : x) }))} />
            <input className={inp} placeholder="Image URL" value={h.image ?? ""} onChange={e => setAbout(a => ({ ...a, hosts: (a.hosts ?? []).map((x, j) => j === i ? { ...x, image: e.target.value } : x) }))} />
          </div>
        ))}
      </Section>

      <Section title="Marani">
        <input className={inp} placeholder="Marani title" value={about.marani_title ?? ""} onChange={e => set("marani_title", e.target.value)} />
        <textarea className={`${inp} min-h-28`} placeholder="Marani body" value={about.marani_body ?? ""} onChange={e => set("marani_body", e.target.value)} />
        <input className={inp} placeholder="Marani image URL" value={about.marani_image ?? ""} onChange={e => set("marani_image", e.target.value)} />
      </Section>

      <Section title="Village">
        <input className={inp} placeholder="Village title" value={about.village_title ?? ""} onChange={e => set("village_title", e.target.value)} />
        <textarea className={`${inp} min-h-28`} placeholder="Village body" value={about.village_body ?? ""} onChange={e => set("village_body", e.target.value)} />
        <input className={inp} placeholder="Village image URL" value={about.village_image ?? ""} onChange={e => set("village_image", e.target.value)} />
      </Section>

      <Section title="Final CTA">
        <input className={inp} placeholder="CTA title" value={about.cta_title ?? ""} onChange={e => set("cta_title", e.target.value)} />
        <textarea className={`${inp} min-h-20`} placeholder="CTA body" value={about.cta_body ?? ""} onChange={e => set("cta_body", e.target.value)} />
      </Section>
