...
type Booking = { id: string; name: string; email: string; phone: string | null; booking_date: string; booking_time: string | null; guests: number; experience: string | null; status: string; created_at: string };
...
function Dashboard() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      return { tastings: t.count ?? 0, bookings: b.count ?? 0, pending: pending.count ?? 0, gallery: g.count ?? 0 };
    },
  });
    ...
    <div className="max-w-6xl">
      ...
      <div className="mt-12">
        <div className="flex items-end justify-between mb-5">
          ...
          <Link to="/admin/bookings" className="text-xs uppercase tracking-widest text-muted-foreground hover:text-gold border-b border-border hover:border-gold pb-1">View all</Link>
        </div>
        ...
        {bookings.length === 0 ? (
          ...
          <div className="border border-border/60 rounded-2xl bg-card overflow-hidden">
            ...
            <ul className="divide-y divide-border/60">
              {bookings.map(b => (
                <li key={b.id} className="px-5 py-4 md:grid md:grid-cols-12 md:gap-4 md:items-center">
                  <div className="md:col-span-3">
                    <div className="font-serif text-lg">{b.name}</div>
                    <div className="text-[10px] uppercase tracking-widest text-muted-foreground mt-0.5">{new Date(b.created_at).toLocaleDateString()}</div>
                  </div>
                  <div className="md:col-span-3 text-sm mt-1 md:mt-0">
                    <a href={`mailto:${b.email}`} className="hover:text-gold block truncate">{b.email}</a>
                    {b.phone && <a href={`tel:${b.phone}`} className="hover:text-gold text-muted-foreground text-xs">{b.phone}</a>}
                  </div>
                  <div className="md:col-span-2 text-sm mt-1 md:mt-0">
                    <div>{b.booking_date}{b.booking_time ? ` · ${b.booking_time.slice(0,5)}` : ""}</div>
                    <div className="text-xs text-muted-foreground">{b.guests} guest{b.guests === 1 ? "" : "s"}</div>
                  </div>
                  <div className="md:col-span-2 text-sm text-muted-foreground mt-1 md:mt-0 capitalize">{b.experience ?? "—"}</div>
...
function StatusPill({ status }: { status: string }) {
  ...
  return <span className={`inline-block text-[10px] uppercase tracking-[0.25em] px-3 py-1.5 rounded-full border ${style[status] ?? style.pending}`}>{status}</span>;
}

function Stat({ icon, label, value, accent }: { icon: React.ReactNode; label: string; value: number; accent?: boolean }) {
  return (
    <div className={`rounded-xl p-5 border ${accent ? "border-gold/60 bg-gold/5" : "border-border/60 bg-card"}`}>
