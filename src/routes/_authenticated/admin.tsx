...
function AdminLayout() {
  ...
  const linkCls =
    "flex items-center gap-3 px-4 py-2.5 rounded-md text-sm hover:bg-gold/10 hover:text-gold transition";
  return (
    <div className="min-h-screen bg-background text-foreground flex">
      ...
      <div className="md:hidden fixed top-0 inset-x-0 z-20 bg-background/90 backdrop-blur border-b border-border/60 flex items-center justify-between px-4 py-3">
        <Link to="/admin" className="font-serif text-lg tracking-[0.25em] text-gold">
          SABACHO
        </Link>
        <button
          type="button"
      ...
      <main className="flex-1 p-6 md:p-10 pt-28 md:pt-10 overflow-auto">
        <Outlet />
      </main>
    </div>
  );
