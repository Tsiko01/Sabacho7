...
export function BookingForm({ defaultExperience = "wine" }: { defaultExperience?: string } = {}) {
  ...
  return (
    <form onSubmit={onSubmit} className="grid gap-4 sm:grid-cols-2">
      <input name="name" required placeholder={t("book.name")} className={inp} maxLength={100} />
      <input name="email" type="email" required placeholder={t("book.email")} className={inp} maxLength={255} />
      <input name="phone" type="tel" placeholder={t("book.phone")} className={inp} maxLength={40} />
      <input name="guests" type="number" min={1} max={50} defaultValue={2} placeholder={t("book.guests")} className={inp} />
      <div>
        <label htmlFor="booking_date" className={lbl}>{t("book.date")}</label>
        <input id="booking_date" name="booking_date" type="date" required className={inp} />
      </div>
      <div>
        <label htmlFor="booking_time" className={lbl}>{t("book.time")}</label>
        <input id="booking_time" name="booking_time" type="time" className={inp} />
      </div>
      <select name="experience" defaultValue={defaultExperience} className={`${inp} sm:col-span-2`}>
      ...
      <textarea name="message" rows={4} placeholder={t("book.message")} className={`${inp} sm:col-span-2 resize-none`} maxLength={2000} />
      <button
        disabled={loading}
